...
//    * @param userId
//    */
//    async verifyOTPUserData(data: Trainee, userId: number): Promise<{response}|Error> {
//      const userData = await getRepository(Trainee).find({ where: { id: userId, otp: data.otp  } });
//      let response = {};
...
//   async resendUserLoginOTP(data:Trainee, userId: number): Promise<{response}|Error> {
//     const userData = await getRepository(Trainee).findOne({ where: { id: userId } });
//     let updateData = {};
//     if (userData) {
//       userData.otp = 1234;
//       updateData = await getRepository(Trainee).save(userData);
//     }
//     let response = {};
//     if (!userData) {
//       response = {
